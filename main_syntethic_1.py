...
number_archetypes = 100
sigma = 1

W = np.random.dirichlet(np.ones(number_archetypes),size=rank)
B = np.zeros((number_timeseries,number_archetypes))

index1 = -1
for index in range(number_timeseries):
    if index % (number_timeseries/rank)==0:
        index1 += 1
        #print("index",index)
        #print(index1)
    B[index,:] = W[index1,:] + sigma * random.random()
    B[index,:] /= sum(B[index])
#print(index1)
...
list_w1 = [5,10,20,100]
list_rank1 = [rank]#, 2*rank, 3*rank, 4*rank]
