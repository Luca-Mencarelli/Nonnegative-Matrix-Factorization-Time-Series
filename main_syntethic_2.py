...
'''Data generation'''
rank = 5
number_timeseries = 1000
number_timestamp = 500
number_archetypes = 5
sigma = 1
periodicity = 50

W = np.random.dirichlet(np.ones(rank),size=number_timeseries)
B = np.zeros((number_timeseries,number_archetypes))
#H = np.random.random((number_archetypes,number_timestamp))
#H = np.zeros((number_archetypes,number_timestamp))
#periodicity = 10
series = np.zeros((number_archetypes, periodicity))

for i in range(number_archetypes):
    series[i] = np.random.rand(periodicity)
H = np.tile(series, int(number_timestamp/periodicity))

for i in range(number_archetypes):
    for j in range(number_timestamp):
        H[i,j] = 100*H[i,j] + sigma * random.random()
"""
for index in range(number_timeseries):
    aux=np.tile(W[index,:], int(number_archetypes/rank))
    for index1 in range(np.size(aux)):
        aux[index1] += sigma * random.random()
    B[index,:] = aux/sum(aux)
"""
...
#B = np.random.dirichlet(np.ones(rank),size=number_timeseries)
...
#B = np.ones((number_timeseries, rank))/rank
#B = np.random.dirichlet(np.ones(number_archetypes),size=number_timeseries)
#H = np.random.rand(number_archetypes, number_timestamp)
X_original = np.matmul(B,H)
"""
for i in range(number_timeseries):
    for j in range(number_timestamp):
        X_original[i,j] += sigma * random.random()
"""
print(X_original)
name = "synthetic2_"
...
list_w1 = [periodicity]
list_rank1 = [number_archetypes]#, 2*rank, 3*rank, 4*rank]
periodicity1 = periods_to_forecast
