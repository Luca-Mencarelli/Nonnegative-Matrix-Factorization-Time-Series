...
def projectM(Ma, Matrix, tau_1a, periods_to_forecast):
    n1, n2 = Ma.shape
    M1 = Ma.copy()
    for i in range(n1):
        if tau_1a[i] == 0:
...
def projectM1(Ma, Matrix, U, V, tau_1a, periods_to_forecast, gamma3=1.1):
...
def mnmff(K, X, H_init, W_init, max_iter, tau_1a, periods_to_forecast):
    ...
    M = projectM(M, X, tau_1a, periods_to_forecast)
    ...
    while not_finish:
        ...
        M = projectM1(M_old, X, U, V, tau_1a, periods_to_forecast)
...
def mnmf(K, X, H_init, W_init, tau_1a, periods_to_forecast, max_iter=100):

    nC = X.shape[0]
    dC = X.shape[1]

    M, U, V, objective_val = mnmff(K, X, H_init, W_init, max_iter, tau_1a, periods_to_forecast)
    Err = np.linalg.norm(M - np.dot(V, U))
    Err_perc = np.linalg.norm(M - np.dot(V, U)) / np.linalg.norm(M)
    # print('Error =', Err)
    # print('Error percentage =', Err_perc)
